...
function EnhancedTable() {
  ...
  const { getContacts, createContact, updateContact, deleteContat, items, totalItems, error, errorMessage } = useZustandStore().contact;
  ...
  useEffect(() => {
    getContacts({ page, itemPerPage: rowsPerPage });
  }, [page, rowsPerPage]);
  ...
  const handleDelete = async () => {
    if (deleteItemId) {
      await deleteContat(deleteItemId);
