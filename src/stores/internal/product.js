...
const createInternalProductSlice = (set) => ({
  ...initialState,
  getProducts: async ({ page = 0, itemPerPage = 0, search = '' }) => {
    set(produce((state) => {
      state.isLoading = true;
    }));

    try {
      const response = await get(`admin/products?page=${page+1}&itemPerPage=${itemPerPage}&search=${search}`, {}, setHeaderSession(true))
  ...
  createProduct: async (payload) => {
    ...
    try {
      await post(`admin/products`, payload, setHeaderSession(true, {
...
export default createInternalProductSlice;
